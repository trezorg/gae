...
def split_to_primes(n: int, split: int=3):
    """Split non prime to sum of primes with split number."""
    results = set()
    for x in range(2, n):
        if is_prime(x):
            if split == 2:
                ...
                for value in split_to_primes(n - x, split=2):
...
def get_prime_sum(n: int, number: int=2):
    """Result of sum of primes."""
    return tuple(islice(split_to_primes(n), number))
